...
The local-buffer DoR is in
`test_site_scoring/data/test_site_dor_buffer_3_8km.csv`, the
ecoregion-wide percentile is in
`test_site_scoring/data/test_site_dor_ecoregion_2024.csv`, and the
joined site-level table is in
`test_site_scoring/data/test_site_scores_combined.csv`. The scoring code
is in `src/degree_of_recovery/core.py`, with workflows in
`test_site_scoring/scripts/03_score_buffer_dor.py`,
`04_score_ecoregion_dor.py` and `05_combine_and_summarise.py`.
