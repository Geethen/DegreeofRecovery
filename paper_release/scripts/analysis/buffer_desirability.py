...
gradient (a regenerating site in a more modified landscape genuinely is less
regenerated), so minimising |rho(DoR, GHM)| would optimise away legitimate signal.
...
from _paths import CACHED, OUT_FIGURES, OUT_REPORT  # noqa: E402

# NOTE: this module was ported from v5/scripts/analysis/buffer_desirability.py.
# Only _load_parent_ghm() (reads V5_DATA) is used by the test-site-scoring
# pipeline (step 07); the desirability sweep/plotting functions below are kept
# for provenance but are not re-run by the reproduction scripts.
V5_DATA  = CACHED
V5_PLOTS = OUT_FIGURES
V5_REPORT = OUT_REPORT
...
# the human-modification gradient -- a regenerating site in a more modified landscape
# genuinely is less regenerated -- so driving |rho(DoR, GHM)| to zero would optimise
...
def plot_ghm_scatter(meta: dict, best: pd.Series, out_stem: Path) -> None:
    ...
    for ax, lbl in zip(axes, labels):
        ...
        ax.set_ylabel("Degree of Regeneration")
...
<div class="sub">Degree-of-Regeneration reference sampling &middot; five sampling goals, scored on
...
DoR <em>should</em> correlate with the human-modification gradient &mdash; a regenerating
site in a more modified landscape genuinely is less regenerated &mdash; so driving
...
loss site has <em>lost</em> its degraded cover and is partially regenerating, so it
...
&mdash; a site regenerating within a more human-modified landscape genuinely reads as
less regenerated &mdash; so the point is to confirm the relationship is sensible and
moderate (the score tracks regeneration, not <em>only</em> ambient context), not to drive
it to zero.</p>
{ghm_scatter_html}
