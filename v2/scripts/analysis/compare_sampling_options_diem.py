"""Compare reference sampling strategies — DIEM distance variant.

Drop-in replacement for compare_sampling_options.py with cosine similarity
swapped for DIEM (Dimension Insensitive Euclidean Metric) throughout:

  - farthest-point diversity selection (cosine_distance_matrix → diem_distance_matrix)
  - DoR distance to good/bad reference sets (cosine_dists_to_set → diem_dists_to_set)
  - LOO AUC scoring (inline cosine matrix → diem_distance_matrix)

All other logic, CLI flags, and output columns are identical to the cosine
version so results can be compared directly.  Output filenames are suffixed
with ``_diem`` to avoid overwriting cosine results.
...
# diem_functions.py lives at the repo root; add it to the path so the script
...
def _init_diem(n_dims: int, min_val: float, max_val: float) -> tuple[float, float]:
    """Sample baseline distance statistics for the embedding space.

    Calls DIEM_Stat once at startup; the returned (exp_center, vard) pair is
    reused for every subsequent distance computation.
    """
...
def _diem_from_euclidean(euclidean: np.ndarray, exp_center: float, vard: float,
                          min_val: float, max_val: float) -> np.ndarray:
    """Apply DIEM normalisation to a pre-computed Euclidean distance array."""
    return (max_val - min_val) * (euclidean - exp_center) / vard


def diem_distance_matrix(
    x: np.ndarray,
    exp_center: float,
    vard: float,
    min_val: float,
    ...
    """Symmetric DIEM distance matrix for the N rows of x (shape N×D).

    Bypasses getDIEM's symmetry-zeroing so the full matrix (including
    diagonal) is returned intact for LOO masking.
    """
    from scipy.spatial.distance import cdist  # type: ignore[import]

    euc = cdist(x, x, metric="euclidean")
    return _diem_from_euclidean(euc, exp_center, vard, min_val, max_val)


def diem_dists_to_set(
    x: np.ndarray,
    points: np.ndarray,
    exp_center: float,
    vard: float,
    ...
    Returns a length-N array.
    """
    from scipy.spatial.distance import cdist  # type: ignore[import]

    euc = cdist(x.reshape(1, -1), points, metric="euclidean").ravel()
    return _diem_from_euclidean(euc, exp_center, vard, min_val, max_val)


# ---------------------------------------------------------------------------
# Unchanged from compare_sampling_options.py
# ---------------------------------------------------------------------------
...
def farthest_point_select(
    ...
    dmat = diem_distance_matrix(emb, exp_center, vard, min_val, max_val)
...
def select_indices(
    ...
    if strategy.method == "feature_diverse":
        return farthest_point_select(emb, strategy.max_raw, rng, exp_center, vard, min_val, max_val)
    ...
    if strategy.method == "hybrid":
        ...
        diverse_local = farthest_point_select(thinned, strategy.max_raw, rng, exp_center, vard, min_val, max_val)
...
def loo_median_scores(
    ...
    dmat = diem_distance_matrix(x, exp_center, vard, min_val, max_val)
...
def run_experiment(
    ...
    for pid, parent in df.groupby("parent_id", sort=False):
        ...
        idx_g = select_indices(g, strategy, rng, exp_center, vard, min_val, max_val)
        idx_b = select_indices(b, strategy, rng, exp_center, vard, min_val, max_val)
        g_sel = g.iloc[idx_g]
        b_sel = b.iloc[idx_b]
        ...
        d_g = diem_dists_to_set(x_obs, g_sel[EMBED_COLS].to_numpy(dtype=float), exp_center, vard, min_val, max_val)
        d_b = diem_dists_to_set(x_obs, b_sel[EMBED_COLS].to_numpy(dtype=float), exp_center, vard, min_val, max_val)
        ...
        s = loo_median_scores(x, selected["ref_state"].to_numpy(), exp_center, vard, min_val, max_val)
        mask = ~np.isnan(s)
...
def main() -> None:
    ...
    parser.add_argument(
        "--exhaustive",
        action="store_true",
        help="Run an expanded strategy matrix.",
    )
    parser.add_argument(
        "--test-embeddings",
        default="degreeRecover/data/test_site_alphaearth_2024.parquet",
        help="Test-site embedding parquet for x_obs.",
    )
    parser.add_argument(
        "--use-ref-mean",
        action="store_true",
        help="Use reference mean as x_obs instead of test-site embedding.",
    )
    parser.add_argument(
        "--emb-min",
        type=float,
        default=0.0,
        help="Minimum value in embedding space (passed to DIEM_Stat).",
    )
    parser.add_argument(
        "--emb-max",
        type=float,
        default=1.0,
        help="Maximum value in embedding space (passed to DIEM_Stat).",
    )
    ...
    print(f"Initialising DIEM for N={n_dims}, range=[{args.emb_min}, {args.emb_max}] ...")
    exp_center, vard = _init_diem(n_dims, args.emb_min, args.emb_max)
    ...
    for st in strategies:
        per_parent, summary, selected = run_experiment(
            ...
            vard=vard,
            min_val=args.emb_min,
            max_val=args.emb_max,
            test_emb=test_emb,
        )
    ...
    parent_csv = out_dir / "sampling_strategy_by_parent_diem.csv"
    summary_csv = out_dir / "sampling_strategy_comparison_diem.csv"
    selected_csv = out_dir / "sampling_strategy_selected_points_diem.csv"
    selected_parquet = out_dir / "sampling_strategy_selected_points_diem.parquet"
    meta_json = out_dir / "sampling_strategy_run_meta_diem.json"
    summary_png = plot_dir / "sampling_strategy_comparison_diem.png"
    ...
    with meta_json.open("w", encoding="utf-8") as f:
        json.dump(
            {
                "input": args.input,
                "distance_metric": "diem",
                "emb_min": args.emb_min,
                "emb_max": args.emb_max,
                "diem_exp_center": exp_center,
                "diem_vard": vard,
                "n_boot": args.n_boot,
                "corr_range_m": args.corr_range_m,
                "seed": args.seed,
    ...
    print(f"  {summary_png}")
    print("\nTop strategies by median CI width (DIEM):")
    print(
        summary_out[
            [
                "strategy",
